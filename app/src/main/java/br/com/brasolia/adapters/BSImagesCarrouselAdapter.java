...
public class BSImagesCarrouselAdapter extends RecyclerView.Adapter<BSImagesCarrouselHolder> {
    ...
    public void onBindViewHolder(BSImagesCarrouselHolder holder, int position) {
        if (holder.context instanceof AppCompatActivity) {
            ...
            BSImageStorage.setImageWithPathToImageViewDownloadingIfNecessary(images.get(position), holder.imageView, 0, width, width, null);
        }
        else {
            BSImageStorage.setImageWithPathToImageViewDownloadingIfNecessary(images.get(position), holder.imageView, 0, 200, 200, null);
        }
    }
